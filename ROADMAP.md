...
- [ ] **infra (remaining)** — channel-router, runtime-broker, mcp-gateway are not in `docker-compose.phase6.yaml` today. Their Dockerfiles still use the old per-repo-root `build.context` and would need the same Pattern B treatment when they land in compose. Tracked here as a follow-up — not blocking anything today.
