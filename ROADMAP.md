...
- [ ] **office-ui** — Convert the remaining page-reexport widgets ([IntegrationsWidget](apps/office-ui/src/widgets/builtin/IntegrationsWidget.tsx), [ProjectsWidget](apps/office-ui/src/widgets/builtin/ProjectsWidget.tsx)) to compact tile-sized components. Pattern lives in the three widgets converted alongside the §6.1 modularity work — `useQuery` + `Skeleton` + `useWidgetConfig`. The blocker today is product: which subset of integrations / projects matters most as a sidebar tile? Pick when there's a UX call.
