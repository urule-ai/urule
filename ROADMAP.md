...
- [ ] **office-ui** — Form draft auto-save. Per-form persistence keyed off route + form id; debounced writes; restore on remount with a "discard draft" affordance. Defer until a heavy form (agent creation wizard, package publish) demonstrably loses work.
- [ ] **office-ui** — Undo/redo for form editing. Stack of dirty-state snapshots scoped to the form's react-hook-form context. Defer alongside auto-save.
- [ ] **office-ui** — Real-time notification sounds (configurable). `useUserPrefsStore.notificationSoundsEnabled` is already plumbed; need `<audio>` ref + a small library of approval/error/mention sounds + a wire-up to the toast/event-bus surface that already drives in-app notifications.
