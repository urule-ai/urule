...
1. Create directory `services/my-service/` with:
   ...
   - `tests/`
2. Register in `server.ts`: correlationIdPlugin → metricsPlugin → CORS → rate limit → auth middleware → swagger → error handler → healthz → routes. The correlation-id plugin must be first so all subsequent middleware logs (auth failures, rate-limit rejections) carry the same `x-correlation-id`. Add `/metrics` to the auth middleware's `publicRoutes` list (Prometheus can't auth). In `src/index.ts` (the entrypoint, before `buildServer` is imported), call `initOtel('<service-name>')` so OpenTelemetry auto-instrumentation can hook the HTTP layer at module-load time. Both `metricsPlugin` and `initOtel` come from `@urule/observability`.
