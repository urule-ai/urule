    ...
    <div className="min-h-screen bg-background-dark grid-bg flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-[440px] space-y-6 relative z-10">
        ...
        <div className="glass-panel rounded-xl p-8 space-y-6 neo-shadow">
          ...
          {/* OAuth providers — Keycloak Identity Provider aliases. The
              actual upstream OAuth handshake is done by Keycloak; this
              UI just sends the user to /protocol/openid-connect/auth
              with kc_idp_hint=<alias>. Configure the providers in the
              Keycloak admin console (alias must match the values below). */}
          ...
          {/* Sign in with SSO — generic Keycloak chooser path (no
              kc_idp_hint), so the user picks from any IdP the operator
              has wired up (SAML, OIDC, etc.). */}
          <button
            type="button"
            onClick={() => signInWith()}
