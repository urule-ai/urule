...
// Mirror packagehub's server.ts publicRoutes config. Catalog browsing is
// public via the method-qualified `GET /api/v1/packages` entry; writes under
// the same prefix (POST publish, version bumps) authenticate.
const PUBLIC_ROUTES = ['/healthz', 'GET /api/v1/packages', '/docs'];
...
async function buildAuthClosedApp() {
  ...
  app.post('/api/v1/packages', async () => ({ id: 'p1' }));
...
describe('packagehub — fail-closed auth wiring', () => {
  ...
  it('keeps GET /api/v1/packages and its sub-paths public (catalog browse)', async () => {
    const app = await buildAuthClosedApp();
  ...
  it('requires auth for POST /api/v1/packages (publish is not public)', async () => {
    const app = await buildAuthClosedApp();
    const res = await app.inject({ method: 'POST', url: '/api/v1/packages', payload: {} });
    expect(res.statusCode).toBe(401);
  });
});
