...
describe('packagehub — x-correlation-id propagation via real buildServer', () => {
  const FAKE_CONFIG = {
    port: 0,
    host: '127.0.0.1',
  ...
  it('echoes the inbound x-correlation-id header on the response', async () => {
    ...
    const res = await app.inject({
      method: 'GET',
      url: '/healthz',
      headers: { 'x-correlation-id': 'test-packagehub-abc-123' },
    });
    expect(res.statusCode).toBe(200);
    expect(res.headers['x-correlation-id']).toBe('test-packagehub-abc-123');
    await app.close();
  });
