...
// Mirror packages' server.ts publicRoutes config. Drift in either direction
// (a route accidentally protected, or accidentally made public) shows up here.
...
async function buildAuthClosedApp() {
  ...
  app.post('/api/v1/packages/install', async () => ({ id: 'p1' }));
  return app;
}

describe('packages — fail-closed auth wiring', () => {
  it('returns 401 on a protected route with no auth header', async () => {
    const app = await buildAuthClosedApp();
    const res = await app.inject({ method: 'POST', url: '/api/v1/packages/install', payload: {} });
