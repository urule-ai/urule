...
// Mirror state's server.ts publicRoutes config. Drift in either direction
...
async function buildAuthClosedApp() {
  ...
  app.post('/api/v1/rooms', async () => ({ id: 'r1' }));
  return app;
}

describe('state — fail-closed auth wiring', () => {
  it('returns 401 on a protected route with no auth header', async () => {
    const app = await buildAuthClosedApp();
    const res = await app.inject({ method: 'POST', url: '/api/v1/rooms', payload: {} });
